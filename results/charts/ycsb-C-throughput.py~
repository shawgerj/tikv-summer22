...
c = canvas(ctype, title='ycsb-B-throughput', dimensions=['6.5in', '5in'])

t_tikv = table(file='../default/b/throughput.txt')
t_tikv_nowal = table(file='../nowal/b/throughput.txt')

d = drawable(canvas=c, xrange=[5, 120], yrange=[0, 18000], yscale='linear',
             coord=['0.7in', '0.6in'], dimensions=['5.7in', '4in'])

axis(drawable=d, title='YCSB Workload B - Throughput',
